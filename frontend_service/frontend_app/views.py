...
def _cart_fetch(user_id, token=None):
    """Lấy giỏ hàng từ cart_service, trả về dict {cart_key: item}."""
    headers = {'Authorization': f'Bearer {token}'} if token else {}
    try:
        resp = requests.get(f"{CART_SERVICE_URL}cart/", params={'user_id': user_id}, headers=headers, timeout=3)
...
def _cart_upsert(user_id, product_id, product_type, product_name, price, quantity, size='', token=None, **kwargs):
...
def _cart_update_qty(user_id, product_id, product_type, quantity, size='', token=None):
            ...
            for item in resp.json():
                if (item['product_id'] == str(product_id)
                    ...
                    requests.patch(
                        ...
                        json={'quantity': quantity}, headers=headers, timeout=3
                    )
                    return
    except Exception as e:
...
def _cart_delete_item(user_id, product_id, product_type, size='', token=None):
...
def _cart_clear(user_id, token=None):
    """Xóa toàn bộ giỏ hàng của user trong cart_service."""
    headers = {'Authorization': f'Bearer {token}'} if token else {}
    try:
        requests.delete(f"{CART_SERVICE_URL}cart/clear/", params={'user_id': user_id}, headers=headers, timeout=3)
    except Exception as e:
...
def home_view(request):
    ...
    import json
    return render(request, 'frontend_app/public/home.html', {
...
def search_view(request):
    ...
    try:
        ...
        SearchLog.objects.create(**payload)
    except Exception as e:
    ...
    return render(request, 'frontend_app/public/search_results.html', {
        'products': products, 
        'query': q,
        'ai_recommendations': ai_recommendations
    })
...
def login_view(request):
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                db_cart = _cart_fetch(user_id, token=data.get('access'))
                ...
                return render(request, 'frontend_app/auth/login_success.html', {
    ...
    return render(request, 'frontend_app/auth/login.html')
...
def register_view(request):
    ...
    return render(request, 'frontend_app/auth/register.html')
...
def profile_view(request):
    ...
    return render(request, 'frontend_app/customer/profile.html', {'user_info': user_info})
...
def cart_view(request):
    ...
    if user_id:
        cart = _cart_fetch(user_id, token=request.session.get('access_token'))
    ...
    return render(request, 'frontend_app/customer/cart.html', {
        'cart': cart, 
        'total': total,
        'ai_recommendations': ai_recommendations
    })
...
def dashboard_view(request):
    ...
    return render(request, 'frontend_app/admin/dashboard.html', {
        'grouped_products': grouped_products,
        'catalogs': catalogs,
...
def product_action_view(request, product_type):
    ...
    if request.method == 'POST':
        ...
        if action == 'delete':
            token = request.session.get('access_token')
            headers = {'Authorization': f'Bearer {token}'} if token else {}
            requests.delete(f"{PRODUCT_SERVICE_URL}{product_id}/", headers=headers)
            ...
            if action == 'add':
                resp = requests.post(PRODUCT_SERVICE_URL, json=data, headers=headers)
            else:
                # Xóa catalog khỏi data khi edit để tránh lưu đè nhầm (do select bị ẩn)
                if 'catalog' in data:
                    del data['catalog']
                resp = requests.patch(f"{PRODUCT_SERVICE_URL}{product_id}/", json=data, headers=headers)
...
def admin_customers_view(request):
    ...
    page_number = request.GET.get('page')
    page_obj = paginator.get_page(page_number)
    return render(request, 'frontend_app/admin/admin_customers.html', {'customers': page_obj})
...
def admin_staff_view(request):
    ...
    staff = []
    try:
        resp = requests.get(f"{USER_SERVICE_URL}admin/staff/", headers=headers, timeout=5)
        if resp.status_code == 200:
            staff = resp.json()
    except Exception as e:
    ...
    page_number = request.GET.get('page')
    page_obj = paginator.get_page(page_number)
    return render(request, 'frontend_app/admin/admin_staff.html', {'staff': page_obj})
...
def admin_stats_view(request):
    ...
    import json
    return render(request, 'frontend_app/admin/admin_stats.html', {
...
def chat_view(request):
    return render(request, 'frontend_app/shared/chat.html')
...
def product_detail_view(request, p_type, p_id):
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            return render(request, 'frontend_app/public/product_detail.html', {'p': normalized})
    except Exception as e:
...
def track_click_api(request):
    ...
    if request.method == 'POST':
        try:
            ...
            InteractionLog.objects.create(**data)
            return JsonResponse({'status': 'ok'})
        except Exception as e:
...
def export_analytics_view(request):
    ...
    try:
        resp = requests.get(NGINX_TRACKING_URL, timeout=5)
...
def bulk_insert_interactions_api(request):
    ...
    if request.method == 'POST':
        try:
            data = json.loads(request.body)
            for item in data:
                InteractionLog.objects.create(**item)
    ...
    if role not in ['staff', 'admin']:
        try:
            ...
            if body.get('status') != 'cancelled':
                return JsonResponse({'error': 'Bạn chỉ có quyền hủy đơn hàng.'}, status=403)
            ...
            if check_resp.status_code == 200:
                ...
                if str(order_data.get('user_id')) != str(user_id):
                    return JsonResponse({'error': 'Không có quyền thao tác trên đơn hàng này.'}, status=403)
            else:
                return JsonResponse({'error': 'Không tìm thấy đơn hàng.'}, status=404)
        except Exception:
            return JsonResponse({'error': 'Lỗi xác thực quyền.'}, status=500)
...
def checkout_view(request):
    ...
    return render(request, 'frontend_app/customer/checkout.html', {'cart_json': cart_json})
...
def order_success_view(request):
    ...
    return render(request, 'frontend_app/customer/order_success.html', {
        'order_id': order_id,
...
def my_orders_view(request):
    ...
    return render(request, 'frontend_app/customer/my_orders.html', {
        'orders': orders,
...
def my_order_detail_view(request, order_id):
    ...
    # Fetch reviews for this order to check if products are already reviewed
    reviews = []
    try:
        review_url = f"{REVIEW_SERVICE_URL}"
        review_resp = requests.get(review_url, params={'order_id': order_id, 'user_id': user_id}, timeout=3)
        if review_resp.status_code == 200:
            reviews = review_resp.json()
    except Exception as e:
        print(f"Error fetching reviews: {e}")

    # Create a mapping of product_id -> review
    reviewed_products = {str(r.get('product_id')): r for r in reviews}
    for item in order.get('items', []):
        item['review'] = reviewed_products.get(str(item.get('product_id')))

    return render(request, 'frontend_app/customer/my_order_detail.html', {'order': order})
...
def shipper_dashboard_view(request):
    ...
    return render(request, 'frontend_app/shipper/shipper_dashboard.html', {
        'shipments': shipments
    })
