...
class CustomerDetailView(generics.RetrieveUpdateDestroyAPIView):
    """
    GET    /api/user/admin/customers/<id>/ → Xem chi tiết Customer
    PATCH  /api/user/admin/customers/<id>/ → Cập nhật thông tin Customer
    DELETE /api/user/admin/customers/<id>/ → Soft delete (is_active=False)
    """
    ...
    def destroy(self, request, *args, **kwargs):
        ...
        if not customer.is_active:
            return Response(
                {'message': f'Tài khoản {customer.username} đã bị vô hiệu hóa trước đó.'},
                status=status.HTTP_400_BAD_REQUEST
            )
        customer.is_active = False
        customer.save(update_fields=['is_active'])
        return Response(
            {'message': f'Đã vô hiệu hóa tài khoản Khách hàng "{customer.username}".'},
            status=status.HTTP_200_OK
        )
